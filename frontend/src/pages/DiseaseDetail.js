...
const DiseaseDetail = () => {
  ...
  const fetchDisease = async () => {
    try {
      ...
      // A 404 means this slug genuinely is not in the catalog; anything else is
      // a connectivity or server problem and should say so rather than
      // pretending the condition does not exist.
      setError(
        err.response?.status === 404
          ? 'Disease not found.'
          : err.response?.data?.message ||
              'Could not load this condition. Please check your connection and try again.'
      );
    } finally {
      setLoading(false);
    }
