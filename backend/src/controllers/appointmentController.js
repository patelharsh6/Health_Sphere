...
const bookAppointment = async (req, res) => {
  try {
    ...
    // GET /api/doctors already hides unverified doctors, but that only removes
    // them from the directory — without this check anyone holding a profile id
    // could still book a doctor whose medical license was never approved,
    // which defeats the whole verification gate.
    ...
    // The partial unique index on { doctor, date, time } is what actually wins
    // the double-booking race; the earlier findOne check only narrows it.
    if (error.code === 11000) {
      throw ApiError.conflict('This time slot is already booked. Please choose another.');
    }
    throw error;
  }
};
...
module.exports = {
  bookAppointment: asyncHandler(bookAppointment),
  getMyAppointments: asyncHandler(getMyAppointments),
  getAppointmentById: asyncHandler(getAppointmentById),
  updateAppointment: asyncHandler(updateAppointment),
  cancelAppointment: asyncHandler(cancelAppointment),
  getTodayAppointments: asyncHandler(getTodayAppointments),
  rescheduleAppointment: asyncHandler(rescheduleAppointment),
  confirmAppointment: asyncHandler(confirmAppointment),
  completeAppointment: asyncHandler(completeAppointment),
  getAppointmentReceipt: asyncHandler(getAppointmentReceipt),
};
