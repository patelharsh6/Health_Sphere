const express = require('express');
const router = express.Router();
const { symptomCheck, getDiseaseBySlug, getAllDiseases, getDiseaseCategories, getDiseaseDoctors } = require('../controllers/aiController');

// POST /api/ai/symptom-check
router.post('/symptom-check', symptomCheck);

// GET /api/ai/diseases/categories
router.get('/diseases/categories', getDiseaseCategories);

// GET /api/ai/diseases
router.get('/diseases', getAllDiseases);

// GET /api/ai/diseases/:slug
router.get('/diseases/:slug', getDiseaseBySlug);

// GET /api/ai/diseases/:slug/doctors
router.get('/diseases/:slug/doctors', getDiseaseDoctors);

module.exports = router;
